...
module.exports = function (RED) {
    function StatusStorerNode(config) {
        ...
        let buffer = [];
        let totalPayloadsSentInSequence = 0; // Tracks items for the current msg.parts sequence
        let lastMessageReceived = null; // Stores the last msg for output context

        let currentIntervalId = null; // Manages the periodic flush timer

        // --- Helper Functions ---

        // Starts or restarts the periodic flush interval.
        ...
        node.on("input", function (msg) {
            // Dynamically update backend URL and token from incoming HTTP request (msg.req.body/headers)
            ...
            lastMessageReceived = msg; // Store the last message for output context
            addToBuffer(msg);
        });

        // Initialize periodic flush on deploy/restart
        startFlushInterval();
        node.status({ fill: "grey", shape: "ring", text: "Initialized and ready." });

        // On node close (e.g., redeploy or Node-RED shutdown)
        node.on("close", async function (done) {
            clearInterval(currentIntervalId); // Clear regular interval
            node.status({ fill: "blue", shape: "ring", text: "Closing..." });

            // Attempt to send any remaining buffered items as a final flush
            if (buffer.length > 0) {
                node.warn(`[StatusStorer] Node closing. Sending ${buffer.length} remaining buffered items.`);
                try {
                    await sendToBackend(lastMessageReceived || {}, true); // Force final send
            ...
            done(); // Call done() when all cleanup is complete
        });
    }
